...
static struct switch_ctx *
alloc_switch_ctx(struct dm_target *ti, unsigned nr_paths, unsigned region_size) {
...
static int alloc_region_table(struct dm_target *ti, unsigned nr_paths) {
    ...
    sctx->region_entries_per_slot =
        (sizeof(region_table_slot_t) * 8) / sctx->region_table_entry_bits;
...
static void switch_get_position(struct switch_ctx *sctx,
...
static void
switch_region_table_write(struct switch_ctx *sctx, unsigned long region_nr, unsigned value) {
    unsigned long region_index;
    unsigned bit;
...
static int process_set_region_mappings(struct switch_ctx *sctx, unsigned argc, char **argv) {
    unsigned i;
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (unlikely(region_index >= sctx->nr_regions)) {
            DMWARN("invalid set_region_mappings region number: %lu >= %lu",
                   region_index,
                   sctx->nr_regions);
            return -EINVAL;
        }
 ...
 */
static int
switch_message(struct dm_target *ti, unsigned argc, char **argv, char *result, unsigned maxlen) {
